...
##程序说明
源程序只有简单实现，经过几次的Thoughtworks的培训，加入了TDD和CI的技术，在原有代码的基础上进行了重构，实现了购物清单的打印操作。

主要文件说明：

- index.html    
    - 主页，可以进行添加购物车、删除购物车中商品、结账产出小票的操作。
- js/*.js
    - action.js  主要实现的js文件，包含初始化，更新购物车，出小票的主要程序
    - jsondata.js  初始化用到的商品信息，活动商品列表以及初始购物车商品列表
- testjs/*.js
    - cartSpec.js 购物车的添加和删除操作测试
    -  receiptSpec.js     小票的产出测试，测试各种优惠的组合是否符合题目要求

使用方法：

- clone 到本地后直接打开index.html 即可操作
...
- npm test //打开火狐浏览器测试
```
程序编写人员：魏璐 ——北京邮电大学 - 网络技术研究院 - 计算机科学与技术

技术指导：王韬 ——ThoughtWorks
